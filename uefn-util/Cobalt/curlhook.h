...
inline CURLcode(*CurlSetOptEOS)(struct Curl_easy*, CURLoption, va_list) = nullptr;
inline CURLcode(*CurlEasySetOptEOS)(struct Curl_easy*, CURLoption, ...) = nullptr;
...
inline CURLcode CurlSetOpt_EOS(struct Curl_easy* data, CURLoption option, ...)
{
	va_list arg;
	va_start(arg, option);

	const auto result = CurlSetOptEOS(data, option, arg);
...
inline CURLcode CurlEasySetOptDetourEOS(struct Curl_easy* data, CURLoption tag, ...)
{
	va_list arg;
	va_start(arg, tag);
	...
	if (tag == CURLOPT_SSL_VERIFYPEER)
	{
		result = CurlSetOpt_EOS(data, tag, 0);
	}
	...
	else if (tag == CURLOPT_URL)
	{
		...
		result = CurlSetOpt_EOS(data, tag, url.c_str());
	}

	else
	{
		result = CurlSetOptEOS(data, tag, arg);
	}
